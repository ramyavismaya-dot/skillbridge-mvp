...
export default async function JobDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
        <div className="space-y-6">
              <div>
                      <Link href="/employer" className="text-sm text-teal hover:underline">&larr; All roles</Link>Link>
                      <h1 className="text-2xl font-bold text-navy mt-1">{job.title}</h1>h1>
                      <p className="text-gray-500 text-sm">{job.company}</p>p>
                {!aiEnabled() && (
              ...
              <div className="card">
                      <h2 className="font-semibold text-navy mb-2">Ranked candidates ({withNarratives.length})</h2>h2>
                {withNarratives.length === 0 && (
                    <p className="text-gray-600 text-sm">
                                No students in the database yet. <Link href="/student" className="text-teal underline">Add one via the assessment</Link>Link>.
                    </p>p>
                      )}
                      ...
                      <div key={r.student.id} className="py-3 space-y-1">
                                    <div className="flex justify-between items-baseline">
                                                    ...
                                                    <span className="text-teal font-semibold text-sm">{Math.round(r.score * 100)}% match</span>span>
                                    </div>div>
                                    <p className="text-sm text-gray-500">{r.student.email}</p>p>
                        {r.narrative && <p className="text-sm text-gray-700">{r.narrative}</p>p>}
                        {!r.narrative && (
