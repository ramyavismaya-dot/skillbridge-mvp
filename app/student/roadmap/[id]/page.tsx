...
export default function RoadmapPage({ params }: { params: { id: string } }) {
  ...
  return (
        <div className="max-w-2xl mx-auto space-y-8">
              <div>
                      <h1 className="text-2xl font-bold text-navy">Your Training Roadmap</h1>h1>
                      <p className="text-gray-500 text-sm">
                                Toward: <span className="font-medium text-gray-800">{topRole?.role.title}</span>span> ({Math.round((topRole?.score ?? 0) * 100)}% current match)
                      </p>p>
              </div>div>
          ...
          {roadmap.length === 0 ? (
                    ...
                    {roadmap.map((item) => (
                                <div key={item.skill} className="card space-y-2">
                                              <div className="flex justify-between items-baseline">
                                                              <h2 className="font-semibold text-navy">{item.skillLabel}</h2>h2>
                                                              <span className="text-xs text-gray-500">gap: {item.gap} level{item.gap > 1 ? "s" : ""}</span>span>
                                              </div>div>
                                              <ul className="space-y-1">
                                                {item.courses.map((c) => (
                                                    <li key={c.title} className="text-sm text-gray-700 flex justify-between border-b last:border-0 py-1">
                                                                        <span>{c.title} <span className="text-gray-400">· {c.provider}</span>span></span>span>
                                                                        <span className="text-gray-500">{c.hours}h</span>span>
                                                    </li>li>
                                                  ))}
                                              </ul>ul>
                                </div>div>
              ...
              <div className="flex gap-3">
                      <Link href={`/student/report/${student.id}`} className="btn-secondary">Back to report</Link>Link>
                      <Link href="/employer" className="btn-primary">See employer view</Link>Link>
              </div>div>
        </div>div>
      );
}
